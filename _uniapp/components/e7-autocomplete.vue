<template>
	<view class="cu-form-group">
		...
		<input class="uni-input" :id="id" :placeholder="fld.placeholder" :value="myValue" @focus="handleInput" @input="handleInput" autocomplete="off"/>
		<text class='cuIcon-roundclosefill' v-if="curInputValue!='' && clearable" @click="handleClear"></text>
		<!-- <view class="str-auto-complete-container-popper__arrow"></view> -->
...
export default {
	...
	data() {
		return {
			...
			needShow: false,
			myValue :this.valueReal || this.value ,
	...
	created() {
		...
		if (this.valueReal) {
			this.myValue = this.valueReal;
			// this.$emit('input', this.valueReal);
		}
		(function() {
			var PinYin = {
				a: '\u554a\u963f\u9515',
	...
	methods: {
		...
		getData(value) {
			...
			const task = this.defultLoadData(value ,this.fld);
			if (task && task.then) {
		...
		selectThisItem(item) {
			...
			this.needShow = false;
			this.curInputValue = item.orginalString;
			
			this.myValue = item.orginalString;
...
.str-auto-complete-container-popper__arrow{
	z-index: 999;
	position: absolute;
	display: block;
	width: 0;
...
.str-auto-complete-container-popper__arrow:after{
	position: absolute;
	display: block;
	width: 0;
	height: 0;
