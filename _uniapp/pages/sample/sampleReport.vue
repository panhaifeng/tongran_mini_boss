<template>
	<view>
		<cu-custom bgColor="bg-gradual-pink" :isBack="true">
			...
			<block slot="content">索样样品排行</block>
		...
		<view class="cu-list menu" >
			<view class="cu-item" v-for="(row,index) in rows" :key="index">
				<view class="content padding-tb-sm">
					<view>
						...
						<text class="text-df margin-right-xs">{{row.proCode}}</text>
					</view>
					<view class="flex  text-df">
						<view class="flex-sub margin-right-xs">名称：{{row.proName}}</view>
						<view class="flex-sub margin-right-xs">颜色：{{row.color}}</view>
					</view>
					<view class="flex  text-df">
						<view class="flex-sub margin-right-xs">门幅：{{row.menfu}}</view>
						<view class="flex-sub margin-right-xs">克重：{{row.kezhong}}</view>
					</view>
					<view class="flex  text-df">
						<view class="flex-sub margin-right-xs">规格：{{row.guige}}</view>
					</view>
					<view class="flex text-df">
						<view class="flex-sub margin-right-xs">成分：{{row.chengfen}}</view>
					</view>
					<view class="flex  text-df">
						<view class="flex-sub margin-right-xs">索样数量：{{row.sNum}}</view>
					</view>
				</view>
			...
			<view class="uni-loadmore text-gray text-center text-df solid-bottom padding" v-if="showLoadMore">{{loadMoreText}}</view>
			<view class="uni-loadmore text-gray text-center text-df solid-bottom padding" v-if="nothing">没有找到数据</view>
		</view>
	...
	export default {
			...
			return {
				...
				btnGroupTop:[
					{text:'高级搜索',icon:'search',handle:()=>{
						...
						let url = '/pages/search/sampleReport'
		...
		methods: {
			...
			loadData() {
				...
				var params = {
					method:'exhibition.sample.samplereport',
				};
				for(var k in param) {
					var _k = k;
