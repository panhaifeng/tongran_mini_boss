<template>
	<view>
		<cu-custom bgColor="bg-gradual-pink" :isBack="true">
			...
			<block slot="content">员工接待概况</block>
		</cu-custom>
		...
		<view class="cu-list menu" >
			<view class="cu-item" v-for="(row,index) in rows" :key="index">
				<view class="cu-avatar round lg bg-blue margin-right-xs">
					<text class="text-df">{{row.realName||"姓名"}}</text>
				</view>
				<view class="content padding-tb-sm">					
					<view class="flex  text-df">
						<view class="flex-sub margin-right-xs">客户数量：{{row.clientCount}}</view>
						<view class="flex-sub margin-right-xs">索样数量：{{row.sampleCount}}</view>
					</view>					
					<view class="flex  text-df">
						<view class="flex-sub margin-right-xs">手机号码：{{row.tel || ''}}</view>
					</view>
					<view class="flex  text-df">
						<view class="flex-sub margin-right-xs">邮件地址：{{row.mail || ''}}</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			loadData() {
				...
				var params = {
					method:'exhibition.sample.exhreport',
				};
				for(var k in param) {
					var _k = k;
