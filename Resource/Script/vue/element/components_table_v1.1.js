...
//点击弹出dialog的链接
//点击时需要从row中带入不同参数
Vue.component('comp-dialog-tablelist-link',{
  props : {
    title :{default: '双击选择'},
    action :{required:true},//必须参数
    // 如果要在sontpl中对弹框的open事件进行自定义,需要定义name属性,然后对callback['mydialog:open']进行定义即可
  ...
  data : function() {
    ...
    if(this.params) {
      for (var k in this.params) {
        // debugger;
        var col = this.params[k].split('.')[1];
        p.push(`${k}=${this.row[col]}`);
      }
    }
    return {
      dialog : null,
  ...
  methods : {
    handleClick :function() {
      // dump(this);
      //从this.paramsGet 中获取参数 to do
      var url = this.action;
      for(var i=0;this.paramsGet[i];i++) {
        url += "&"+this.paramsGet[i];
      }
      this.dialog.setAction(url);
      this.dialog.show();
    }
  },
  mounted : function() {
    //在$root里注册comp-dialog-tablelist组件
    var opt = {
      type : 'comp-dialog-tablelist',
      //必须提供name参数,方便返回实例
    ...
    this.$root.makeNewComp(opt);
    // this.
    this.$nextTick(()=>{
      this.dialog = this.$root.$refs[opt.name][0];
    });
    
    // dump('dialog-link',this);
  }
});