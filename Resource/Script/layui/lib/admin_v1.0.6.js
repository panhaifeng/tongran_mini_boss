...
;layui.define("view", function(e) {
    var a = layui.jquery
      ...
      , F = {
        ...
        theme: function(e) {
            ...
            layui.data(l.tableName))
              , n = "LAY_layadmin_theme11"
              , s = document.createElement("style")
              , r = e.color.alias.indexOf("white")>=0 ? i([".layui-side-menu,", ".layadmin-pagetabs .layui-tab-title li:after,", ".layadmin-pagetabs .layui-tab-title li.layui-this:after,", ".layui-layer-admin .layui-layer-title,", ".layadmin-side-shrink .layui-side-menu .layui-nav>.layui-nav-item>.layui-nav-child", "{background-color:{{d.color.main}};}", ".layui-nav-tree .layui-this,", ".layui-nav-tree .layui-this>a,", ".layui-nav-tree .layui-nav-child dd.layui-this,", ".layui-nav-tree .layui-nav-child dd.layui-this a", "{background-color:{{d.color.selected}} !important;}", ".layui-layout-admin .layui-logo{background-color:{{d.color.logo || d.color.main}} !important;}", "{{# if(d.color.header){ }}", ".layui-layout-admin .layui-header{background-color:{{ d.color.header }};}", ".layui-layout-admin .layui-header a,", ".layui-layout-admin .layui-header a cite{color: #f8f8f8;}", ".layui-layout-admin .layui-header a:hover{color: #fff;}", ".layui-layout-admin .layui-header .layui-nav .layui-nav-more{border-top-color: #fbfbfb;}", ".layui-layout-admin .layui-header .layui-nav .layui-nav-mored{border-color: transparent; border-bottom-color: #fbfbfb;}", ".layui-layout-admin .layui-header .layui-nav .layui-this:after, .layui-layout-admin .layui-header .layui-nav-bar{background-color: #fff; background-color: rgba(255,255,255,.5);}", ".layadmin-pagetabs .layui-tab-title li:after{display: none;}",".layui-layer-admin .layui-layer-title{background-color: {{d.color.header}} !important;}  .layui-nav-tree .layui-nav-child dd.layui-this a{color: #fff !important;} .layui-nav-itemed>.layui-nav-child{background-color: #f5f5f5 !important;}  .layui-nav-itemed>a, .layui-nav-tree .layui-nav-title a, .layui-nav-tree .layui-nav-title a:hover {color: #000 !important;}  .layui-nav-tree .layui-nav-child a{color:#000;} .layui-nav .layui-nav-item a{color:#000;}  .layui-nav .layui-nav-more{border-color: #aaa transparent transparent;border-top-color: rgba(90,90,90,.7);}  .layui-nav .layui-nav-mored, .layui-nav-itemed>a .layui-nav-more{border-color: transparent transparent #999;}  .layui-side-menu .layui-nav .layui-nav-item a:hover{color:#000;}  .layui-layout-admin .layui-logo, .layui-layout-admin .layui-logo a{color:#000 !important;} .layui-header .layui-nav-item .layui-icon{color:#fff;}  .layui-nav-item.layui-this a{color:#fff !important;}", "{{# } }}"].join("")).render(e = a.extend({}, t.theme, e)) : i([".layui-side-menu,", ".layadmin-pagetabs .layui-tab-title li:after,", ".layadmin-pagetabs .layui-tab-title li.layui-this:after,", ".layui-layer-admin .layui-layer-title,", ".layadmin-side-shrink .layui-side-menu .layui-nav>.layui-nav-item>.layui-nav-child", "{background-color:{{d.color.main}};}", ".layui-nav-tree .layui-this,", ".layui-nav-tree .layui-this>a,", ".layui-nav-tree .layui-nav-child dd.layui-this,", ".layui-nav-tree .layui-nav-child dd.layui-this a", "{background-color:{{d.color.selected}} !important;}", ".layui-layout-admin .layui-logo{background-color:{{d.color.logo || d.color.main}} !important;}", "{{# if(d.color.header){ }}", ".layui-layout-admin .layui-header{background-color:{{ d.color.header }};}", ".layui-layout-admin .layui-header a,", ".layui-layout-admin .layui-header a cite{color: #f8f8f8;}", ".layui-layout-admin .layui-header a:hover{color: #fff;}", ".layui-layout-admin .layui-header .layui-nav .layui-nav-more{border-top-color: #fbfbfb;}", ".layui-layout-admin .layui-header .layui-nav .layui-nav-mored{border-color: transparent; border-bottom-color: #fbfbfb;}", ".layui-layout-admin .layui-header .layui-nav .layui-this:after, .layui-layout-admin .layui-header .layui-nav-bar{background-color: #fff; background-color: rgba(255,255,255,.5);}", ".layadmin-pagetabs .layui-tab-title li:after{display: none;}", "{{# } }}"].join("")).render(e = a.extend({}, t.theme, e))
      ...
      , P = F.events = {
        ...
        flexibleLogo: function(e) {
            var i = e.hasClass('spread-logo');
            if(!i){
                e.addClass('spread-logo');
            }else{
                e.removeClass('spread-logo');
            }
            F.sideFlexible(i ? "spread" : null)
        },
        refresh: function() {
            var tabUrl = a("#LAY_app_tabsheader").find('li.layui-this').attr('lay-id');
            var e = ".layadmin-iframe"
              , i = a("." + b).length;
            F.tabsPage.index >= i && (F.tabsPage.index = i - 1);
            var t = F.tabsBody(F.tabsPage.index).find(e);
            if(tabUrl){
                t[0].contentWindow.location.href = tabUrl
            }else{
        ...
        theme: function() {
            F.popupRight({
                ...
                success: function() {
                    n(this.id).render("tpl/system/theme")
                }
            })
        },
        note: function(e) {
        ...
        about: function() {
            F.popupRight({
                ...
                success: function() {
                    n(this.id).render("http://www.eqinfo.com.cn/",{} ,false)
                }
            })
        },
        more: function(e) {
    ...
    o.on("mouseenter", ta, function() {
        ...
        a('.tab-main-left-btn').addClass('display-block');
        a('.tab-main-right-btn').addClass('display-block');
        a('.layadmin-pagetabs-custom').addClass('layadmin-pagetabs-custom-left');
    }),
    o.on("mouseleave", ta, function() {
        var t = a("#LAY_app_tabsheader")
              , l = t.children("li")
              , n = (t.prop("scrollWidth"),
            t.outerWidth())
              , s = parseFloat(t.css("left"));
        if ((!s && s <= 0))return;
        a('.tab-main-left-btn').removeClass('display-block');
        a('.tab-main-right-btn').removeClass('display-block');
        a('.layadmin-pagetabs-custom').removeClass('layadmin-pagetabs-custom-left');
    }),
