 ...
 * @package Core
 * @version $Id: FLEA.php 1031 2008-02-03 08:51:28Z qeeyuan $
 */
...
// 定义 FleaPHP 版本号常量
define('FLEA_VERSION', '1.0.70.1031');
    ...
    */
    ...
    function & getDBO($dsn = 0)
    {
        ...
        if (!is_array($dsn) || !isset($dsn['driver'])) {
            ...
            $ret = __THROW(new FLEA_Db_Exception_InvalidDSN($dsn));
            return $ret;
        }
    ...
    function initSetSession(){
        // 设置session的保存路径
        $sessTime = defined('sessionSaveTime') ? sessionSaveTime : 3600 * 2;//默认2个小时
        if(defined('sessionSaveHandler') && strtolower(sessionSaveHandler) == 'redis'){
            ini_set('session.save_handler' , 'redis');
            $tmpPath = defined('sessionSavePath') ? sessionSavePath : "tcp://127.0.0.1:6379";
        }else{
            $dsn      = FLEA::getAppInf('dbDSN');
            $sPath    = defined('sessionSavePath') ? sessionSavePath : $dsn['database'];

            $sessionpath = session_save_path();
            $tmpPath = $sessionpath.DS.md5($sPath);
            if(!is_dir($tmpPath)){
                mkdir($tmpPath ,0777);
            }
        }
        ...
        // ini_set("session.cookie_lifetime",$sessTime);
    }
}
 ...
 * md5扩展的md5加密
 * 加盐
 * Time：2017/08/28 13:25:17
 * @author li
*/
function _md5($string ,$prex='易奇_' ,$after='_e7_eqinfo'){
    return md5(md5($prex.$string).$after);
}


/**
 * 获取ip的方法
 * Time：2018/04/26 16:22:50
 * @author li
 * @param 参数类型
 * @return 返回值类型
*/
...
/**
 * 系统日志，需要有Model_Sys_Log
 *
 * @param string $content
 *
 * @return string
 */
function sysDbLog($array = array() ,$model = '' ,$mdlName = '' ,$compare = false)
{
    $mdlLog = FLEA::getSingleton('Model_Sys_Log');
    return $mdlLog->log($array ,$model ,$mdlName ,$compare);
}
...
/**
 * @desc ：结合chromephp输出php变量到chrome的console中，方便调试
 * @author jeff 2017/12/21 14:54:00
 * @param 参数类型
 * @return 返回值类型
*/
...
{
    $content = "\r\n".print_r($vars,1);
    file_put_contents('debug_log.txt', $content,FILE_APPEND);
    return null;
}
