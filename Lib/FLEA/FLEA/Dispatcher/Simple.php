...
class FLEA_Dispatcher_Simple
{
    ...
    {
        //2015-9-2 by jeff,增加对api调用的判断
        $temp = explode('/',$_SERVER['SCRIPT_NAME']);
        $fileName = $temp[count($temp)-1];
        //判断是否api测试
        if(strpos($fileName, 'apitest.php')!==false) {
            $class_test = & FLEA::getSingleton('Api_Test');
            $class_test->test();//列出所有api
            return true;
        }
        //判断是否api调用
        if(strpos($fileName, 'api.php')!==false) {
            $class_api = & FLEA::getSingleton('Api_Response');
            $ret = $class_api->response();
            return $ret;
        }
        ...
        //判断是否api调用
        if(strpos($fileName, 'crontab.php')!==false) {
            $controllerName = 'Crontab';
            $actionName = 'Run';
        }elseif(strpos($fileName, 'queue.php')!==false){
            $controllerName = 'Crontab';
            $actionName = 'RunQueueList';
        }elseif(strpos($fileName, 'doQueue.php')!==false){
            $controllerName = 'Crontab';
            $actionName = 'RunQueue';
        }
