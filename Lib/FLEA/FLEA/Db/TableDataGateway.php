 ...
 * @package Core
 * @version $Id: TableDataGateway.php 1029 2008-02-02 06:47:09Z qeeyuan $
 */
...
class FLEA_Db_TableDataGateway
{
    ...
    var $force_master_key = "/*FORCE_MASTER*/";
    ...
    function & findAll($conditions = null, $sort = null, $limit = null, $fields = '*', $queryLinks = true)
    {
        ...
        // 如果需要强制使用主实例获取数据，添加sql标识
        if($this->force_master == true){
            $sql = " {$this->force_master_key} ".$sql;
        }
    ...
    function update(& $row, $saveLinks = true)
    {
        ...
        //打日志
        sysDbLog($row,$this,($this->moduleName?$this->moduleName.':修改':$this->tableName.':修改'),true);
    ...
    function create(& $row, $saveLinks = true)
    {
        ...
        //打日志
        sysDbLog($row,$this,($this->moduleName?$this->moduleName.':新增':$this->tableName.':新增'));
    ...
    function removeByPkv($pkv)
    {
        ...
        //查询准备删除的数据，放到日志里
        if(!$row){
            $row = $this->find(array($this->primaryKey => $pkv));
        }
        ...
        sysDbLog($row,$this,($this->moduleName?$this->moduleName.':删除':$this->tableName.':删除'));
    ...
    function removeByConditions($conditions)
    {
        ...
        $this->dbo->startTrans();
        foreach ($rowset as $row) {
            if (!$this->removeByPkv($row[$this->primaryKey], false)) { break; }
            $count++;
        }
        $this->dbo->completeTrans();
    ...
    function execute($sql, $inputarr = false)
    {
        ...
        sysDbLog(array('SQL'=>$sql,'url'=>$url),$this,$url.':execute执行sql语句');
    ...
    function getWhere($conditions, $queryLinks = true) {
        ...
        do {
            ...
            foreach ($linksWhere as $linkid => $lws) {
				...
				//重大bug发现,会导致多个link叠加的条件产生问题,update by jeff 2011-10-28
				$where = substr($whereby, 6);
                unset($link);
            }
    ...
    function _updateCounterCache(& $row)
    {
        foreach (array_keys($this->links) as $linkKey) {
            ...
            $sql = "UPDATE {$link->assocTDG->qtableName} SET {$f} = (SELECT COUNT(*) FROM {$this->qtableName} WHERE {$link->qforeignKey} = {$fkv}) WHERE {$link->assocTDG->qpk} = {$fkv}";
