...
class FLEA_Db_Driver_Sqlite
{
    ...
    function connect($dsn = false)
    {
		...
		if(file_exists($dsn['db'])){
			$this->conn=sqlite_open($dsn['db']);
			if($this->conn){
				return $this->conn;
			}
		}
		FLEA::loadClass('FLEA_Db_Exception_SqlQuery');
       __THROW(new FLEA_Db_Exception_SqlQuery("connect('{$dsn['db']}') failed!"));
		return false;
    }
    ...
    {
        return sqlite_num_rows($this->conn);//这里仅对select有效
    }
    ...
    {
        $row = sqlite_fetch_array($res,SQLITE_ASSOC);//Sqlite 没有这个功能
        $temp = array();
        foreach($row as $key => $value){
        	$key = eregi_replace('^[a-z0-9_]+\.','',$key);
        	$temp[$key] = $value;
