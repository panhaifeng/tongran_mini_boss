 ...
 * @package Core
 * @version $Id: TableLink.php 1012 2007-11-09 00:13:20Z qeeyuan $
 */
...
class FLEA_Db_BelongsToLink extends FLEA_Db_TableLink
{
    ...
    function getFindSQL($in)
    {
        ...
        //查找主表的对应字段优先取belongsTo中p_primaryKey，没有再获取qpk by lwj 2015-9-9
        $_assocTDG_PK = $this->p_primaryKey != '' ? $this->p_primaryKey : $this->assocTDG->qpk;

        $sql = "SELECT {$fields} FROM {$this->assocTDG->qtableName} LEFT JOIN {$this->mainTDG->qtableName} ON {$this->mainTDG->qpk} {$in} WHERE {$this->qforeignKey} = {$_assocTDG_PK} ";
        $in = '';
        return parent::_getFindSQLBase($sql, $in);
    }
