...
class FLEA_Helper_Pager
{
    ...
    function FLEA_Helper_Pager(& $source, $currentPage, $pageSize = 20, $conditions = null, $sortby = null, $basePageIndex = 0)
    {
        ...
        if (is_object($source)) {
            ...
            //2013-5-29 comment by jeff,这个方法太费时间，改为使用SQL_CALC_FOUND_ROWS来取得
            //如果语句中没有union 加上一个limit 1,可以更加省时间
            $sql = trim($source);
            $sql = rtrim($sql, ';');
            $sql = str_replace("\r\n", "\n", $sql);
            $sql = str_replace("\r", "\n", $sql);
            ...
            // dump(strpos($sql," from "));
            if(strpos(strtolower($sql)," from ")===false) {
                echo "sql语句中必须有from!";
                exit;
            }
            //如果语句没有union,加个limit 1;
            if(strpos(strtolower($sql),' union ')===false) {
                $sql .= " LIMIT 1";
            }

            //$fromPos = strpos($text,' from ');
            $sql = substr($sql,0,7).' SQL_CALC_FOUND_ROWS '.substr($sql,7);
            //SQL_CALC_FOUND_ROWS为mysql特有，所以直接mysql_query

            //系统如果定义为不显示错误信息，则显示一个已经定义的有好界面，否则显示错误信息
			if(defined('ERROR_SYS_DIR')){
			   require_once(ERROR_SYS_DIR);
			   $error=new TMIS_ErrorSys();
				mysql_query($sql) or $error->_die("sql语句有错,语句为:<br />{$this->source}<br />错误：<font color='red'>".mysql_error()."</font>",'数据访问失败');
			}else{
				mysql_query($sql) or die("sql语句有错,语句为:<br />{$this->source}<br />错误：<font color='red'>".mysql_error()."</font>");
			}
            $re =  mysql_fetch_row(mysql_query("SELECT FOUND_ROWS()"));
            $this->totalCount = $this->count = (int)$re[0];
