...
<body>
  <div class="" id="div1">
      ...
      <el-main>
        <el-form
          ...
          <comp-card
            ...
            <template slot="body">
              <el-col
                ...
                :key="fld.name"
                :span="(fld.type=='comp-textarea' || fld.type=='comp-message-alert')?24:8"
                >
                <el-form-item
                  :label="fld.title"
                  :prop="fld.name"
          ...
          <comp-card
            :card-width-col="formWidthCol"
            :card-height="formHeight"
            :card-title="titleOther"
            >
            <template slot="body">
              <!-- 子表区域 -->
              <comp-table
                :data="row[sonOtherKey]"
                :cols="columnsSonOther"
                border
                height="300"
                ref="sonTblOther"
                @row-dblclick = "handleEditSonOther"
                @row-click = "handleRowClick"
                @cell-mouse-enter="handelMouseOverRow"
                @cell-mouse-leave="handelMouseLeaveRow"
                @index-header-click="handleAddSonOther"
                >
                <!-- 操作栏的效果-下拉菜单或者平铺按钮 -->
                <template slot="rowButtonSlot" slot-scope="props">
                  <!-- 前三个按钮组作为默认显示的按钮 -->
                  <el-button
                    v-for="(btn,index) in defaultEditButtons2"
                    :key="index"
                    type="primary"
                    plain
                    :icon="btn.icon?btn.icon:defaultEditButtonsIcons[index]"
                    ...
                    size="mini"
                    style="padding:3.5px;margin-left:3px;"
                    @click="handleRowCommandOther({btn:btn,row:props.row,index:props.index})"
                    :title="btn.text"
                    :disabled="btn.options && props.row[btn.options.disabledColumn]"
                    ></el-button>
                  <!-- 后面的按钮以dropDown呈现 -->
                  <el-dropdown
                    size='small'
                    @command="handleRowCommandOther"
                    trigger="click"
                    @visible-change="showDropdown=arguments[0];showDropdownIndex=props.index;"
                    v-if="moreEditButtons2.length>0">
                    <el-button type="primary" plain icon="el-icon-more" circle size="mini" style="padding:3.5px;margin-left:3px;"></el-button>
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item
                        v-for="(btn,index) in moreEditButtons2"
                        :key="index"
                        :command="{btn:btn,row:props.row,index:props.index}"
                        :disabled="btn.options && props.row[btn.options.disabledColumn]"
                        >{{btn.text}}</el-dropdown-item>
    ...
    <el-dialog title="明细编辑" :visible.sync="dialogSonVisible"
      @open="handleOpen('first')"
      @close="handleClose('first')"
      ref="dialogSon">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button-group>
          <el-button type="plain" icon="el-icon-arrow-left" @click="handlePre('first')">上一条</el-button>
          <el-button type="plain" icon="el-icon-arrow-right" @click="handleNext('first')">下一条</el-button>
    ...
    <!-- 明细信息编辑弹窗 -->
    <el-dialog :title="titleOther" :visible.sync="dialogSonOtherVisible"
      @open="handleOpen('other')"
      @close="handleClose('other')"
      ref="dialogSonOther">
      <el-form :model="rowSonOther" size="mini">
        <el-form-item
          v-for="(item,index) in sonOtherFormItems"
          :key="index"
          :label="item.title"
          label-width="100px"
          >
          ...
          <component
            :is="item.type"
            :fld="item"
            :value="rowSonOther[item.name]"
            @input="rowSonOther[item.name]=arguments[0]"
            @displaytext-change="setRowSonOther(item.textKey||item.displayKey,arguments[0])"
            @displaytext-set="setDisplaytext(arguments[0],rowSonOther[item.textKey||item.displayKey])"
            :ref="item.name"
            @change="changeSelect"
            @changeimg="changeImg"
          ></component>
      ...
      <div slot="footer" class="dialog-footer">
        <!-- <el-button type="primary" @click="handleSonOk">确 定</el-button> -->
        <el-button-group>
          <el-button type="plain" icon="el-icon-arrow-left" @click="handlePre('other')">上一条</el-button>
          <el-button type="plain" icon="el-icon-arrow-right" @click="handleNext('other')">下一条</el-button>
          <el-button @click="dialogSonOtherVisible = false" icon="el-icon-close">关闭</el-button>
        </el-button-group>
      </div>
    </el-dialog>
...
<script>
  var title='页面标题';
  var titleOther='<{$titleOther}>';
  var row = <{$row|@json_encode}>;
  var mainFormItems = <{$mainFormItems|@json_encode}>;
  var sonFormItems = <{$sonFormItems|@json_encode}>;
  var sonOtherFormItems = <{$sonOtherFormItems|@json_encode}>;
  var rules = <{$rules|@json_encode}>;
  var action = "<{$action}>";
  var callbacks = [];
  var sonKey = <{$sonKey|@json_encode}>;
  var sonTplMap = {
    'first':{'dialog':'dialogSon','dataName':'sonKey','row':'rowSon','sonFormItems':'sonFormItems'},
    'other':{'dialog':'dialogSonOther','dataName':'sonOtherKey','row':'rowSonOther','sonFormItems':'sonOtherFormItems'},
  };
...
<{/if}>
  ...
  //对字表数据集进行预处理
      ...
      var sonOtherButtons = <{$sonOtherButtons|@json_encode}>||[
        {text:'删除',funcName:'handleDeleteOther'},
        {text:'复制',funcName:'handleCopyOther'},
      ];

      //将 editButtons 分解成两个部分,前3个一组默认显示,后面的以dropdown呈现
      var defaultEditButtons = [];
      var defaultEditButtons2 = [];
      var moreEditButtons = [];
      var moreEditButtons2 = [];
      ...
      sonOtherButtons.forEach((item,i)=>{
        if(i<3) defaultEditButtons2.push(item);
        else {
          moreEditButtons2.push(item);
        }
      });
      ...
      var columnsSonOther = <{$columnsSonOther|@json_encode}>||{};
      for(var k in columnsSonOther) {
        if(columnsSonOther[k].showButton) {
          columnsSonOther[k].width = columnsSonOther[k].width>130 ? columnsSonOther[k].width : 130;
        }
      }
      return {
        'formWidthCol':'',
        ...
        'sonOtherFormItems':sonOtherFormItems,
        'row':row,
        //记录集中代表子表记录的字段名
        'sonKey':sonKey,
        'sonOtherKey':<{$sonOtherKey|@json_encode}>,
        //子表记录编辑表单是否可见
        'dialogSonVisible':false,
        'dialogSonOtherVisible':false,
        //子表列
        'columnsSon':columnsSon,
        'columnsSonOther':columnsSonOther,
        //子表弹窗绑定的记录集,
        'rowSon':this.getEmptyRowSon(sonFormItems),
        'rowSonOther':this.getEmptyRowSon(sonOtherFormItems),
        //当前编辑的子表记录的index
        'indexSon':-1,
        //验证规则
        'rules':rules,
        ...
        'defaultEditButtons2':defaultEditButtons2,
        //从第4个开始后面的操作按钮
        'moreEditButtons':moreEditButtons,
        'moreEditButtons2':moreEditButtons2,
        //每行功能按钮的可用图标
    ...
    methods : {
      ...
      changeSelect : function(val,obj) {
        ...
        // if(this.columnsSon[k].displayKey && this.sonFormItems[k].type=='comp-select')


        // dump(this.columnsSon,this.sonFormItems);
        // for(var k in this.columnsSon) {
        //   if(this.columnsSon[k].displayKey && this.sonFormItems[k].type=='comp-select') {
        //     dump(oldVal,newVal);
        //   }
        // }
      ...
      setRowSonOther : function(key,text) {
        // rowSon[item.textKey||item.displayKey]=arguments[0]
        this.rowSonOther[key] = text;
      },
      ...
      //每行的编辑菜单项选中
      handleRowCommandOther({btn,row,index}) {
        //自定义组件
        if(btn.type=='comp') {
          var options = btn.options;
          if(!options.name) {
        ...
        //定义每个typ默认的handle
        var arrMap = {
          edit:{funcName:'handleEditSonOther'},
          remove:{funcName:'handleDeleteOther'},
          copy:{funcName:'handleCopyOther'},
          func:{funcName:false},
          comp:{funcName:false},
        };
        // debugger;
      ...
      //复制
      handleCopyOther : function(row,index) {
        var r = JSON.parse(JSON.stringify(row));
        //删除id属性,避免重复id出现
        delete r.id
        this.row[this.sonOtherKey].splice(index+1,0,r);
      },
      //增加子表记录
      ...
      handleAddSonOther : function() {
        var   len = this.row[this.sonOtherKey].length;
        this.row[this.sonOtherKey].push({});
        this.rowSonOther=this.row[this.sonOtherKey][len];

        for(var key in this.sonOtherFormItems) {
          var temp = '';
          var item = this.sonOtherFormItems[key];
          if(item.type=='comp-checkbox-group' ||
            item.type=='comp-pop-multi-select' ||
            item.type=='comp-image' ||
            item.type=='comp-file' ||
          ...
          // dump(key);
          Vue.set(this.rowSonOther,key,temp);
        }

        this.indexSon = this.row[this.sonOtherKey].length - 1;
        this.dialogSonOtherVisible=true;
      },
      //插入空白行
      ...
      //子表编辑弹窗打开时回调
      handleOpen : function(handleKey) {
        // if(handleKey == undefined){
        //   handleKey = 'index';
        // }
        //这里可加入用户自定义的dialogSon:open回调函数,
        var event = sonTplMap[handleKey]['dialog']+':open';
        var dataKey = sonTplMap[handleKey]['dataName'];
        var rowKey = sonTplMap[handleKey]['row'];

        dataKey = this[dataKey];

        var curRow = this.row[dataKey][this.indexSon];
        var rowSon2 = this[rowKey];
        // debugger;
        //更改弹窗中特殊控件的属性
        ...
        // if(handleKey == 'index'){
          this.$nextTick(()=>{
            var sonFormItemsTemp = this[sonTplMap[handleKey]['sonFormItems']];
            for (var k in sonFormItemsTemp) {
              var item = sonFormItemsTemp[k];
        ...
        if(this.callbacks[event]) {
          ...
          if(!curRow) curRow={};
          this.callbacks[event].apply(this,[rowSon2,curRow]);
        }
      },
      //子表编辑弹窗关闭时回调
      handleClose : function(handleKey) {
        //这里可加入用户自定义的dialogSon:open回调函数,
        var event = sonTplMap[handleKey]['dialog']+':close';
        var dataKey = sonTplMap[handleKey]['dataName'];
        dataKey = this[dataKey];
        if(this.callbacks[event]) {
          var curRow = this.row[dataKey][this.indexSon];
          if(!curRow) curRow={};
          this.callbacks[event].apply(this,[curRow]);
        }
      },
      //子记录弹窗点击上一条
      handlePre: function(handleKey) {
        if(this.indexSon==0) {
          alert('已经是第一条');
          return;
        }
        ...
        var rowKey = sonTplMap[handleKey]['row'];
        this.indexSon--;
        this[rowKey] = this.row[dataKey][this.indexSon];

        var event = sonTplMap[handleKey]['dialog']+':handlePre';
        if(this.callbacks[event]) {
          this.callbacks[event].apply(this,[this[rowKey]]);
        }
      },
      //子记录弹窗点击下一条
      handleNext: function(handleKey) {
        var dataKey = sonTplMap[handleKey]['dataName'];
        dataKey = this[dataKey];
        var rowKey = sonTplMap[handleKey]['row'];
        // debugger
        var len = this.row[dataKey].length;
        var sonFormItemsTemp = this[sonTplMap[handleKey]['sonFormItems']];
        // debugger;
        if(this.indexSon==len-1) {
          if(this.hideButtonAddSon) {
            ...
            this.$confirm('已经是最后一条,点击"确认",新增一条记录!').then(_=>{
              this.row[dataKey].push({});
              this[rowKey]=this.row[dataKey][len];

              for(var key in sonFormItemsTemp) {
                Vue.set(this[rowKey],key,'');
              }

              this.indexSon = this.row[dataKey].length - 1;
              return ;
            }).catch(_=>{
              return;
            });
        ...
        var event = sonTplMap[handleKey]['dialog']+':handleNext';
        if(this.callbacks[event]) {
          this.callbacks[event].apply(this,[this[rowKey]]);
        }
      },
      ...
      //表格的编辑按钮点击
      handleEditSonOther : function(row,index) {
        //双击时没有传入index,需要遍历得到index
        if(typeof(index)!='number') {
          this.row[this.sonOtherKey].every((r,i)=>{
            if(r.id==row.id) {
              index=i;
              return false;//终止循环,注意必须是every才会终止
            }
      ...
      //表格的删除按钮点击事件
      handleDeleteOther : function(row,index) {
        //阻止事件冒泡
        this.$confirm('确认删除吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          ...
          //ajax从后台删除记录,todo
          // debugger;
          var url=<{$urlRemoveSonOther|json_encode}>;
          if(url==null) {
            console.error('必须定义模版变量urlRemoveSonOther');
            return;
          }
          var params = {id:row.id};
          // dump(this.rowSon)
          ...
          .then((res)=>{
            ...
            //从数据集中删除当前记录
            this.row[this.sonOtherKey].splice(index,1);
          }).catch(function (error) {
            dump(error);
          });
    ...
    mounted: function(){
      //隐藏表格边线
      var initTpl = ['sonTbl','sonTblOther'];
      for(var ktpl in initTpl){
        var kt  = initTpl[ktpl];
        var objRefs = this.$refs[kt];
        // debugger;
        objRefs.border=false;
        // this.$refs.traderId[0].fld.options=[];
        //设置是否显示操作列
        objRefs.showEditColumn=true;
        //显示合计行
        objRefs.showSummary = true;
        //不显示操作栏文字,显示按钮
        objRefs.showHeaderText = false;
        //设置显示序号列
        objRefs.showIndex = true;
        objRefs.showHeaderIndexButton = !this.hideButtonAddSon;
      }
    ...
    created : function() {
      ...
      this.$bus.$on('selectChanged', (key,opts) => {
        // debugger;
        if(!this.dialogSonOtherVisible) return;
        if(!key) return;
        if(!this.columnsSonOther[key].displayKey) return;
        var t = [];
        ...
        t = t.join(',');
        // debugger;
        // this.rowSon[this.columnsSonOther[key].displayKey] = t;
        // var row = this.row[this.sonKey][this.indexSon];
        this.rowSonOther[this.columnsSonOther[key].displayKey] = t;
        // dump(this.rowSonOther);
      });
    }
  });
</script>
